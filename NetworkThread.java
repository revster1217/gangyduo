...
public class NetworkThread extends Thread {
    ...
    private void parseMessage(String message) {
        ...
        switch (parts[0]) {
            ...
            case "OPPONENT_MOVED":
                // "OPPONENT_MOVED:<shipName>:<row>:<col>:<H|V>"
                // The opponent moved one of their ships — but we don't show
                // the opponent's ships on our screen (enemy grid hides ships),
                // so we only need to update the server-side grid via handleMove.
                // Nothing to draw on our end — just acknowledge.
                if (parts.length == 5) {
                    System.out.println("Opponent moved " + parts[1]
                            + " to [" + parts[2] + "][" + parts[3] + "] " + parts[4]);
                }
                break;
