...
public class GameServer {
    ...
    // =========================================================================
    //  HANDLE MOVE
    //  Updates the server-side grid and broadcasts the new position to the opponent.
    // =========================================================================
    private synchronized void handleMove(int movingPlayerIndex, String shipName,
                                         int newRow, int newCol, boolean isHorizontal) {
        Grid movingGrid = grids[movingPlayerIndex];

        // Find the ship by name in the moving player's grid
        Ship targetShip = null;
        for (Ship ship : movingGrid.getShips()) {
        ...
        if (targetShip == null) {
            System.out.println("MOVE: ship not found: " + shipName);
            return;
        }

        // The client already validated the move locally before sending it.
        // We trust the client and directly apply the new position on the server.
        // Re-validating here would cause sync issues — especially since hit tiles
        // on the ship's old position would cause isValidPlacement to return false.
        movingGrid.removeShipTrace(targetShip);
        targetShip.setPlacement(newRow, newCol, isHorizontal);
        movingGrid.addShipTrace(targetShip);

        // Tell the opponent where this ship moved so their screen updates
        int opponentIndex = 1 - movingPlayerIndex;
        String orientation = isHorizontal ? "H" : "V";
        connections.get(opponentIndex).sendMessage(
                "OPPONENT_MOVED:" + shipName + ":" + newRow + ":" + newCol + ":" + orientation);
    }
    ...
    private class ConnectionThread extends Thread {
        ...
        private void parseMessage(String message) {
            ...
            switch (parts[0]) {
                ...
                case "MOVE":
                    // Format: "MOVE:<shipName>:<row>:<col>:<H|V>"
                    if (parts.length == 5) {
                        String shipName = parts[1];
                        int row = Integer.parseInt(parts[2]);
                        int col = Integer.parseInt(parts[3]);
                        boolean isHorizontal = parts[4].equals("H");
                        handleMove(playerIndex, shipName, row, col, isHorizontal);
                    }
                    break;
