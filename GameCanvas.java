...
public class GameCanvas extends JComponent {
    ...
    // -------------------------------------------------------------------------
    //  KEY BINDINGS — movement (from original), rotation, and ammo selection
    // -------------------------------------------------------------------------
    private void setupAmmoButtons() {
        InputMap  im = this.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW);
        ActionMap am = this.getActionMap();

        // --- Ship movement (same as original) ---
    ...
    // -------------------------------------------------------------------------
    //  SHIP MOVEMENT — copied from original, plus sends MOVE message to server
    // -------------------------------------------------------------------------

    // The ship the player controls with arrow keys.
    // Set by GameStarter after ships are placed.
    ...
    private void moveShip(int dRow, int dCol) {
        ...
        localPlayer.getOceanGrid().removeShipTrace(controlledShip);

        if (localPlayer.getOceanGrid().isValidPlacement(
                controlledShip, newRow, newCol, controlledShip.isHorizontal())) {
            controlledShip.setPlacement(newRow, newCol, controlledShip.isHorizontal());
        }

        localPlayer.getOceanGrid().addShipTrace(controlledShip);
        repaint();

        // Tell the server (and therefore the opponent) the new position
        sendMoveMessage();
    }
    ...
    private void rotateShip() {
        ...
        if (localPlayer.getOceanGrid().isValidPlacement(
                controlledShip, controlledShip.getStartRow(),
                controlledShip.getStartCol(), newOrientation)) {
            controlledShip.setPlacement(
                    controlledShip.getStartRow(),
                    controlledShip.getStartCol(), newOrientation);
        }
        ...
        // Tell the server the new orientation
        sendMoveMessage();
    }

    // Sends the controlled ship's current position and orientation to the server.
    // Format: "MOVE:<shipName>:<row>:<col>:<H|V>"
    private void sendMoveMessage() {
        try {
            String orientation = controlledShip.isHorizontal() ? "H" : "V";
            String message = "MOVE:" + controlledShip.getName() + ":"
                    + controlledShip.getStartRow() + ":"
                    + controlledShip.getStartCol() + ":"
                    + orientation;
            out.writeUTF(message);
            out.flush();
            System.out.println("Sent: " + message);
        } catch (IOException e) {
            System.out.println("Failed to send move message: " + e.getMessage());
        }
    }
